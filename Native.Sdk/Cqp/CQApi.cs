...
namespace Native.Sdk.Cqp
{
    ...
    public class CQApi
    {
        ...
        public IOTQEventHandler eventhandler;
        ...
        public int SendGroupMessage(long groupId, params object[] message)
        {
            ...
            return webapi.sendTextMessage(groupId, sb.ToString(), MsgType.GROUP).Ret;
        }
        ...
        public QQMessage SendGroupMessage(Model.Group group, params object[] message)
        {
            if (group == null)
            {
                throw new ArgumentNullException("group");
        ...
        public int SendPrivateMessage(long qqId, params object[] message)
        {
            ...
            return webapi.sendTextMessage(qqId, sb.ToString(), MsgType.FRIEND).Ret;
        }
        ...
        {
            throw new NotImplementedException("CQ2IOT 未支持该操作");
        }
        ...
        {
            List<FriendInfo> tempFriends = new List<FriendInfo>();
            throw new NotImplementedException("CQ2IOT 未支持该操作");
            return tempFriends;
        ...
        /// <exception cref="EndOfStreamException">无法读取数据, 因为已读取到数据流末尾</exception>
        /// <returns>获取成功返回 <see cref="GroupMemberInfo"/>, 失败返回 <code>null</code></returns>
        public GroupMemberInfo GetGroupMemberInfo(Model.Group group, QQ qq, bool notCache = false)
        {
            if (group == null)
            {
                throw new ArgumentNullException("group");
        ...
        {
            if (groupId < Model.Group.MinValue)
            {
                throw new ArgumentOutOfRangeException("groupId");
            }
            List<GroupMemberInfo> gminfol = new List<GroupMemberInfo>();
            List<QQUser> qu = webapi.getGroupMemberList(groupId);
            foreach(QQUser q in qu)
            {
                GroupMemberInfo gm = new GroupMemberInfo()
                {
                    Age = q.Age,
                    Card = q.GroupCard,
                    MemberType = (q.GroupAdmin == 1) ? QQGroupMemberType.Manage : QQGroupMemberType.Member,
                    Nick = q.NickName,
                    Level = q.MemberLevel.ToString(),
                    LastSpeakDateTime = new DateTime(),
                    Group = new Model.Group(this, groupId),
                    QQ = new QQ(this, q.MemberUin)
                };
                gminfol.Add(gm);
            }
            return gminfol;
        ...
        /// <exception cref="EndOfStreamException">无法读取数据, 因为已读取到数据流末尾</exception>
        /// <returns>获取成功返回 <see cref="List{GroupMemberInfo}"/>, 失败返回 <code>null</code></returns>
        public List<GroupMemberInfo> GetGroupMemberList(Model.Group group)
        {
            if (group == null)
            {
                throw new ArgumentNullException("group");
