    ...
    internal class DataTypes
    {
        ...
        public int ReadNextVarInt(Queue<byte> cache)
        {
            ...
            while (true)
            {
                ...
                if (j > 5)
                {
                    throw new OverflowException("VarInt too big " + rawData);
                }
        ...
        private byte[] GetNbt(Dictionary<string, object> nbt, bool root)
        {
            ...
            foreach (KeyValuePair<string, object> item in nbt)
            {
                // Skip NBT root name
                if (item.Key == "" && root)
                {
        ...
        public byte[] GetLocation(Location location)
        {
            ...
            {
                locationBytes = BitConverter.GetBytes(((((ulong)location.X) & 0x3FFFFFF) << 38) | ((((ulong)location.Y) & 0xFFF) << 26) | (((ulong)location.Z) & 0x3FFFFFF));
            }
