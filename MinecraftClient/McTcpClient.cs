...
namespace MinecraftClient
{
    ...
    public class McTcpClient : IMinecraftComHandler
    {
        ...
        public void BotUnLoad(ChatBot b)
        {
            ...
            List<KeyValuePair<string, List<ChatBot>>> botRegistrations = registeredBotPluginChannels.Where(entry => entry.Value.Contains(b)).ToList();
            foreach (KeyValuePair<string, List<ChatBot>> entry in botRegistrations)
            {
        ...
        public bool MoveTo(Location location, bool allowUnsafe = false)
        {
            lock (locationLock)
            {
                ...
                {
                    path = Movement.CalculatePath(world, this.location, location, allowUnsafe);
                }
