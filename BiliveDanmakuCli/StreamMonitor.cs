...
#pragma warning disable CS0067 // 从不使用事件“StreamMonitor.RoomViewersUpdate”
        public event RoomViewersUpdateEvt RoomViewersUpdate;
#pragma warning restore CS0067 // 从不使用事件“StreamMonitor.RoomViewersUpdate”
        public event StreamStoppedEvent StreamStopped;
        public event ExceptionHappendedEvent ExceptionHappened;
        ...
        private void Receiver_ReceivedDanmaku(object sender, ReceivedDanmakuArgs e)
        {
            switch (e.Danmaku.MsgType)
            {
                ...
                case MsgTypeEnum.LiveEnd:
                    if (IsMonitoring)
                    {
                        Task.Run(() => StreamStopped?.Invoke(this));
                    }
                    break;
                default:
                    break;
        ...
        private void ConnectWithRetry()
        {
            ...
            {
                Thread.Sleep(Math.Max(2000, 0));
                connect_result = Connect();
            }

            if (connect_result)
            {
                Console.WriteLine("[StreamMonitor]弹幕服务器连接成功");
        ...
        private bool Connect()
        {
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine("[StreamMonitor]连接弹幕服务器错误");
                Task.Run(() => ExceptionHappened?.Invoke(this, "连接弹幕服务器错误\n" + ex.Message + "\n" + ex.StackTrace, ex));
                return false;
            }
        }
        ...
        private void ReceiveMessageLoop()
        {
            ...
            Console.WriteLine("[StreamMonitor]ReceiveMessageLoop");
            try
            {
                    ...
                    void ProcessDanmaku(int action, byte[] local_buffer, int length)
                    {
                        switch (action)
                        {
                            ...
                            case 5://playerCommand
                                string json = Encoding.UTF8.GetString(local_buffer, 0, length);
                                try
                                {
                                ...
                                {
                                    Console.WriteLine("[StreamMonitor]EXCEPTION:\n" + ex.Message + "\n" + ex.StackTrace);
                                }
                                break;
                            default:
                                break;
            ...
            catch (Exception ex)
            {
                ...
                if (!(dmTokenSource?.IsCancellationRequested ?? true))
                {
                    ...
                    Console.WriteLine("[StreamMonitor]弹幕连接中断，试图重连...\n" + ex.Message + "\n" + ex.StackTrace);
                    Task.Run(() => ExceptionHappened?.Invoke(this, "弹幕连接中断，试图重连...\n" + ex.Message + "\n" + ex.StackTrace, ex));
                    ConnectWithRetry();
                }
            }
        }
        ...
        private void SendSocketData(int action, string body = "")
        {
            ...
            byte[] buffer = new byte[(playload.Length + 16)];

            using (MemoryStream ms = new MemoryStream(buffer))
            {
                byte[] b = BitConverter.GetBytes(buffer.Length).ToBE();
                ms.Write(b, 0, 4);
