...
namespace BililiveRecorder.Core
{
    internal static class BililiveAPI
    {
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(cookie_string))
                {
                    try
                    {
                        ...
                        foreach (string[] t in cookie_string.Trim(' ', ';').Split(';').Select(x => x.Trim().Split(new[] { '=' }, 2)))
                        {
                            try
                            {
                        ...
                        HttpClient pclient = new HttpClient(handler: new HttpClientHandler
                        {
                            CookieContainer = cc
                        }, disposeHandler: true)
                        {
                ...
                HttpClient cleanclient = new HttpClient { Timeout = TimeSpan.FromSeconds(5) };
        ...
        public static async Task<string> GetPlayUrlAsync(int roomid, bool avoidtxy = true)
        {
            ...
            if (avoidtxy)
            {
                ...
                while (true)
                {
                    ...
                    {
                        string[] all = all_jtoken.Select(x => x["url"].ToObject<string>()).ToArray();
                        string[] withoutTxy = all.Where(x => !x.Contains("txy.")).ToArray();
            ...
            else
            {
                ...
                {
                    System.Collections.Generic.IEnumerable<string> urls = array.Select(t => t?["url"]?.ToObject<string>());
                    string[] distinct = urls.Distinct().ToArray();
        ...
        public static async Task<RoomInfo> GetRoomInfoAsync(int roomid)
        {
            try
            {
                JObject room = await HttpGetJsonAsync($@"https://api.live.bilibili.com/room/v1/Room/get_info?id={roomid}");
                ...
                JObject user = await HttpGetJsonAsync($@"https://api.live.bilibili.com/live_user/v1/UserInfo/get_anchor_in_room?roomid={roomid}");
